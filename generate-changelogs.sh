...
FULL_CHANGELOG=$(npm x -y --package=conventional-changelog-cli@latest -- conventional-changelog -u -p eslint -r 0)

RECENT_CHANGELOG_OUTPUT="CHANGELOG.md"
RECENT_RELEASE_LIMIT=50
RECENT_CHANGELOG=$(npm x -y --package=conventional-changelog-cli@latest -- conventional-changelog -u -p eslint -r $RECENT_RELEASE_LIMIT)
